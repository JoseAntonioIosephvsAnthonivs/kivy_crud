...
Config.set("graphics","height","640") #height
...
class DataBaseWid(BoxLayout):
    ...
    def check_memory(self):
        ...
        cursor = con.cursor()
        cursor.execute('select ID, Nome, Marca, Valor, Estoque from Productos')
        for i in cursor:
            wid = DataWid(self.mainwid)
            r1 = 'ID: '+str(100000000+i[0])[1:9]+'\n'
            r2 = i[1]+', '+i[2]+'\n'
            r3 = 'Valor por unidade: '+'R$'+str(i[3])+'\n'
            r4 = 'Em estoque: '+str(i[4])
            wid.data_id = str(i[0])
...
class UpdateDataWid(BoxLayout):
    ...
    def check_memory(self):
        con = sqlite3.connect(self.mainwid.DB_PATH)
        cursor = con.cursor()
        s = 'select Nome, Marca, Valor, Estoque from Productos where ID='
        cursor.execute(s+self.data_id)
        for i in cursor:
            self.ids.ti_nome.text = i[0]
            self.ids.ti_marca.text = i[1]
            self.ids.ti_valor.text = str(i[2])
            self.ids.ti_estoque.text = str(i[3])
        con.close()
    ...
    def update_data(self):
        ...
        d3 = self.ids.ti_valor.text
        d4 = self.ids.ti_estoque.text
        a1 = (d1,d2,d3,d4)
        s1 = 'UPDATE Productos SET'
        s2 = 'Nome="%s",Marca="%s",Valor=%s,Estoque=%s' % a1
        ...
        try:
            ...
            self.mainwid.Popup.title = "Banco de dados erro"
            if '' in a1:
...
class InsertDataWid(BoxLayout):
    ...
    def insert_data(self):
        ...
        d4 = self.ids.ti_valor.text
        d5 = self.ids.ti_estoque.text
        a1 = (d1,d2,d3,d4,d5)
        s1 = 'INSERT INTO Productos(ID, Nome, Marca, Valor, Estoque)'
